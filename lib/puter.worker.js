const PRIVATE_PROJECT_PREFIX = "pland_project_private_";
const PUBLIC_PROJECT_PREFIX = "pland_project_public_";
...
router.post("/api/projects/save", async ({ request, user }) => {
  try {
    ...
    if (!userInfo.id) return jsonError(401, "Authentication failed");

    const payload = normalizeProject(project, "private", userInfo);
    const key = makePrivateKey(userInfo.id, project.id);
...
router.post("/api/projects/share", async ({ request, user }) => {
  try {
    ...
    const privateKey = makePrivateKey(userInfo.id, id);
    const project = await userPuter.kv.get(privateKey);
    if (!project) return jsonError(404, "Private project not found");

    const sharedProject = normalizeProject(project, "public", userInfo);
    const publicKey = makePublicKey(userInfo.id, id);

    // Use me.puter for shared data accessible by all users
    await me.puter.kv.set(publicKey, sharedProject);
...
router.get("/api/projects/list", async ({ user }) => {
  try {
    ...
    // Use me.puter to access shared public projects
    const publicProjects = (
      await me.puter.kv.list(PUBLIC_PROJECT_PREFIX, true)
...
router.get("/api/projects/get", async ({ request, user }) => {
  try {
    ...
    // Use me.puter to access shared public projects
    const publicEntries = await me.puter.kv.list(PUBLIC_PROJECT_PREFIX, true);
...
router.post("/api/projects/delete", async ({ request, user }) => {
  try {
    ...
    if (!deleted) {
      ...
      if (!publicProject) return jsonError(404, "Project not found");
      if (publicProject.ownerId !== userInfo.id)
        return jsonError(403, "Unauthorized to delete this project");

      // Use me.puter to delete from shared storage
      deleted = await me.puter.kv.del(publicKey);
    }
